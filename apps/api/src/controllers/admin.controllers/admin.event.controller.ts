...
export class AdminController {
    ...
    async createEvent (req: Request, res: Response) {
        ...
        const createdEvent = await this.adminService.createEvent(eventData, discountPercentage, is_active)
        if (createdEvent) {
            res.status(201).send ({
    ...
    async updateEvent (req: Request, res: Response) {
        ...
        const discountPercentage = Number (req.body.discountPercentage)
        const is_active = Boolean (req.body.is_active)
        const updatedEvent = await this.adminService.updateEvent(id,discount_id,updatedEventData,discountPercentage,is_active)
        if (updatedEvent) {
            res.status(200).send ({
